...
class GroupCandidates:
    ...
    def get_candidate_positions_for_value(self, value: Value) -> list[Position]:
        return [pos for pos in self.candidates[value]] if value in self.candidates else []

    def remove_group_candidate(self, pos: Position, val: Value):
        if val in self.candidates:
            if pos in self.candidates[val]:
