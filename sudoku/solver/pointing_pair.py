...
class PointingPair:
    ...
    @staticmethod
    def validate_pointing_pair_args(group_type: GroupType, group_num: int, positions: tuple[Position, Position], value: Value, eliminated_candidates: list[Cell]) -> None:
        ...
        for candidate in eliminated_candidates:
            if positions[0].row_num == positions[1].row_num:
                assert candidate.pos.row_num == positions[0].row_num
            elif positions[0].col_num == positions[1].col_num:
                assert candidate.pos.col_num == positions[0].col_num
    ...
    def __init__(self, group_type: GroupType, group_num: int, positions: tuple[Position, Position], value: Value, eliminated_candidates: list[Cell]) -> None:
        PointingPair.validate_pointing_pair_args(group_type, group_num, positions, value, eliminated_candidates)
    ...
    def __str__(self):
        return f'Pointing Pair - {self.group_type} {self.group_num} - {self.positions} - {self.value} - Removed Candidate Cells: {self.eliminated_candidates}'
